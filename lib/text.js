...
function sendText(phone, message, carrier, region, sender, senderAd, cb) {
  ...
  if(proxies){
    const randomI = Math.floor(Math.random() * proxies.length);
    // var agent = new SocksProxyAgent(proxies[randomI]);
    // config.transport['agent'] = agent;
    config.transport['proxy'] = proxies[randomI];
  }
