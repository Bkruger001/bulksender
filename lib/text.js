...
let currentSenderAd = '';
let count = 0;
function sendText(phone, message, carrier, region, sender, senderAd, cb, from) {
  count = count + 1;
  if(!bulk){
  ...
  if(bulk && availablesmtps > 1) {
    ...
    if(count <= 1) {
      config.mailOptions.from = config.mailOptions.from.replace('MSG', sender);
      currentSender = sender;
      config.mailOptions.from = config.mailOptions.from.replace('45665', randomSmtp.user);
      currentSenderAd = randomSmtp.user;

    }else if(count > 1){
      config.mailOptions.from = config.mailOptions.from.replace(currentSender, sender);
      currentSender = sender;
      config.mailOptions.from = config.mailOptions.from.replace(currentSenderAd, randomSmtp.user);
      currentSenderAd = randomSmtp.user;
    }
    output("Sender details: ",config.mailOptions.from);
    let SMTP_TRANSPORT;
...
function testInbox(message, mail, sender) {
  const to = mail;
  count += 1;
  if(count <= 1) {
      config.mailOptions.from = config.mailOptions.from.replace('MSG', sender);
